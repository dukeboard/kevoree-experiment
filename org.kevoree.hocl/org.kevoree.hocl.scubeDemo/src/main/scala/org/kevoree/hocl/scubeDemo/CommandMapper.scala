...
class CommandMapper (solution: Solution) {
  ...
  def buildHoclCommand (p: org.kevoreeAdaptation.AdaptationPrimitive /*, nodeName: String*/): HoclCommand = {
    p.getPrimitiveType.getName match {
      ...
      case JavaSePrimitive.StartInstance if (p.getRef.isInstanceOf[ComponentInstance]) => AddComponentMolecule(p.getRef.asInstanceOf[ComponentInstance], solution)
      case JavaSePrimitive.StopInstance if (p.getRef.isInstanceOf[ComponentInstance]) => RemoveComponentMolecule(p.getRef.asInstanceOf[ComponentInstance], solution)
      ...
      case "AddNode" => AddNodeMolecule(p.getRef.asInstanceOf[ContainerNode], solution)
      case "RemoveNode" => RemoveNodeMolecule(p.getRef.asInstanceOf[ContainerNode], solution)
